...
class User_Avatars {
	...
	public function __construct() {
		...
		// Shortcode.
		add_shortcode( 'burcon-outfitters', [ $this, 'shortcode' ] );
	...
	public function admin_init() {
		...
		add_settings_field( 'basic-user-avatars-caps', __( 'Local Avatar Permissions', 'burcon-outfitters' ), [ $this, 'avatar_settings_field' ], 'discussion', 'avatars' );
	...
	public function avatar_settings_field() {
		...
		<label for="burcon_user_avatars_caps">
			...
			<?php _e( 'Only allow users with file upload capabilities to upload local avatars (Authors and above)', 'burcon-outfitters' ); ?>
		</label>
		<?php
	...
	public function edit_user_profile( $profileuser ) {
		...
		<h3><?php _e( 'Avatar', 'burcon-outfitters' ); ?></h3>
		<table class="form-table">
			<tr>
				<th><label for="basic-user-avatar"><?php _e( 'Upload Avatar', 'burcon-outfitters' ); ?></label></th>
				...
				if ( empty( $options['burcon_user_avatars_caps'] ) || current_user_can( 'upload_files' ) ) {
					...
					if ( empty( $profileuser->burcon_user_avatar ) ) {
						echo '<span class="description">' . __( 'No local avatar is set. Use the upload field to add a local avatar.', 'burcon-outfitters' ) . '</span>';
					} else {
						echo '<input type="checkbox" name="basic-user-avatar-erase" value="1" /> ' . __( 'Delete local avatar', 'burcon-outfitters' ) . '<br />';
						echo '<span class="description">' . __( 'Replace the local avatar by uploading a new avatar, or erase the local avatar (falling back to a gravatar) by checking the delete option.', 'burcon-outfitters' ) . '</span>';
					}

				} else {
					if ( empty( $profileuser->burcon_user_avatar ) ) {
						echo '<span class="description">' . __( 'No local avatar is set. Set up your avatar at Gravatar.com.', 'burcon-outfitters' ) . '</span>';
					} else {
						echo '<span class="description">' . __( 'You do not have media management permissions. To change your local avatar, contact the site administrator.', 'burcon-outfitters' ) . '</span>';
					}
				}
				?>
				</td>
	...
	function shortcode() {
		...
		<form id="basic-user-avatar-form" action="<?php the_permalink(); ?>" method="post" enctype="multipart/form-data">
			...
			if ( empty( $options['burcon_user_avatars_caps'] ) || current_user_can( 'upload_files' ) ) {
				...
				if ( empty( $profileuser->burcon_user_avatar ) ) {
					echo '<p class="description">' . __( 'No local avatar is set. Use the upload field to add a local avatar.', 'burcon-outfitters' ) . '</p>';
				} else {
					echo '<input type="checkbox" name="basic-user-avatar-erase" value="1" /> ' . __( 'Delete local avatar', 'burcon-outfitters' ) . '<br />';
					echo '<p class="description">' . __( 'Replace the local avatar by uploading a new avatar, or erase the local avatar (falling back to a gravatar) by checking the delete option.', 'burcon-outfitters' ) . '</p>';
				}

			} else {
				if ( empty( $profileuser->burcon_user_avatar ) ) {
					echo '<p class="description">' . __( 'No local avatar is set. Set up your avatar at Gravatar.com.', 'burcon-outfitters' ) . '</p>';
				} else {
					echo '<p class="description">' . __( 'You do not have media management permissions. To change your local avatar, contact the site administrator.', 'burcon-outfitters' ) . '</p>';
				}
			}
			?>
			<input type="submit" name="manage_avatar_submit" value="<?php _e( 'Update Avatar', 'burcon-outfitters' ); ?>" />
		</form>
		<?php
	...
	public function bbpress_user_profile() {
		...
		echo '<div>';
			echo '<label for="basic-local-avatar">' . __( 'Avatar', 'burcon-outfitters' ) . '</label>';
 			echo '<fieldset class="bbp-form avatar">';
				...
				if ( empty( $options['burcon_user_avatars_caps'] ) || current_user_can( 'upload_files' ) ) {
					...
					if ( empty( $profileuser->burcon_user_avatar ) ) {
						echo '<span class="description" style="margin-left:0;">' . __( 'No local avatar is set. Use the upload field to add a local avatar.', 'burcon-outfitters' ) . '</span>';
					} else {
						echo '<input type="checkbox" name="basic-user-avatar-erase" value="1" style="width:auto" /> ' . __( 'Delete local avatar', 'burcon-outfitters' ) . '<br />';
						echo '<span class="description" style="margin-left:0;">' . __( 'Replace the local avatar by uploading a new avatar, or erase the local avatar (falling back to a gravatar) by checking the delete option.', 'burcon-outfitters' ) . '</span>';
					}

				} else {
					if ( empty( $profileuser->burcon_user_avatar ) ) {
						echo '<span class="description" style="margin-left:0;">' . __( 'No local avatar is set. Set up your avatar at Gravatar.com.', 'burcon-outfitters' ) . '</span>';
					} else {
						echo '<span class="description" style="margin-left:0;">' . __( 'You do not have media management permissions. To change your local avatar, contact the site administrator.', 'burcon-outfitters' ) . '</span>';
					}
				}
